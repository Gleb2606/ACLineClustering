...
# Загрузка данных
X, df_clean = data_scale(file_path, first_parameter, second_parameter, Float.FLOAT32, 0)

# Кластеризация MeanShift
ms = MeanShift(bandwidth=0.8, bin_seeding=True)
clusters = ms.fit_predict(X)

# Визуализация
plot(df_clean, first_parameter, second_parameter, clusters, "MeanShift")
